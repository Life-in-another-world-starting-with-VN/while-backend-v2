...
중요 규칙:
1. 세션은 동적으로 생성되므로 first_session_content만 작성
2. **first_session_content 작성 규칙 (매우 중요!)**:
...
중요 규칙:
1. **빠른 전개**: 한 세션은 3-5개 정도의 짧은 씬으로 구성하세요
...
2. **선택지 활용**: 2-3개 씬마다 선택지를 제공하여 사용자 참여 유도
   - 선택지는 스토리에 영향을 주는 의미있는 선택이어야 함
   - 단순한 yes/no가 아닌 다양한 행동 옵션 제공
   - 장소 이동 관련 선택지를 적극 활용

3. **감정 반영**: 사용자 감정을 고려하여 스토리를 조정하세요
...
2. **자연스러운 반응**:
   - 선택지 이후는 항상 캐릭터나 나레이터의 dialogue로 반응
   - 캐릭터의 성격에 맞는 반응
   - 감정 표현이 자연스럽게
...
4. **감정 반영**: 사용자 감정 {emotion_str}를 고려한 반응
